...
export default function ProductForm({
    ...
    async function addImages(files: FileList | null) {
        ...
        try {
            const uploaded: Awaited<
                ReturnType<typeof uploadThreeDFile>
            >[] = [];

            for (const file of Array.from(files)) {
                const image = await uploadThreeDFile(
                    file,
                    "image",
                    ensureUploadProductId(),
                );
    ...
    async function removeImage(image: ThreeDProductImage) {
        ...
        if (
            image.key &&
            !originalKeys.current.has(image.key)
        ) {
            try {
                await deleteThreeDFile(image.key);
            } catch {
                // The product update remains authoritative.
            }
        }
    }
    ...
    async function addModel(file: File | undefined) {
        ...
        try {
            const uploaded = await uploadThreeDFile(
                file,
                "model",
                ensureUploadProductId(),
            );
            ...
            setError(
                cause instanceof Error
                    ? cause.message
                    : "ไม่สามารถอัปโหลด Model file ได้",
            );
        } finally {
            setUploading(false);
    ...
    async function clearModel() {
        ...
        if (
            key &&
            !originalModelKey.current
        ) {
            try {
                await deleteThreeDFile(key);
            } catch {
                // Cleanup is retried by the save lifecycle.
            }
        }
    }
        ...
        >
                ...
                <div className="grid gap-5 md:grid-cols-2">
                    ...
                    <Field label="รายละเอียด">
                        <textarea
                            ...
                            rows={4}
                            className={`${inputClass} h-auto py-3 md:col-span-2`}
                        />
                    </Field>
                </div>
            </FormSection>
            ...
            >
                ...
                >
                    ...
                    <p className="mt-3 font-bold text-slate-800">
                        ลากรูปมาวาง หรือเลือกจากเครื่อง
                    </p>
                    ...
                    <button
                        ...
                        className="mt-4 inline-flex items-center gap-2 rounded-xl bg-pink-500 px-4 py-2.5 text-sm font-bold text-white hover:bg-pink-600"
                    >
                        <Upload size={16} />
                        เพิ่มรูปภาพ
                    </button>
                            ...
                            (image, index) => (
                                ...
                                >
                                    <div className="relative aspect-[4/3] bg-slate-100">
                                        ...
                                        {index === 0 && (
                                            <span className="absolute left-3 top-3 inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-xs font-bold text-pink-600">
                                                <Star
                                                    size={12}
                                                    fill="currentColor"
                                                />
            ...
            >
                <div className="flex flex-wrap items-center gap-3">
                    <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-700 hover:border-pink-300"
                    >
                        <Upload size={16} />
                        อัปโหลด STL, OBJ, 3MF หรือ ZIP
                    </button>
            ...
            <div className="flex flex-wrap gap-3 border-t border-slate-100 pt-5">
                <button
                    ...
                    className="rounded-xl bg-pink-500 px-5 py-3 text-sm font-bold text-white hover:bg-pink-600 disabled:opacity-50"
                >
                    {saving
                        ? "กำลังบันทึก..."
                        : submitLabel}
...
function FormSection({
    ...
    return (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
            <p className="text-[10px] font-black tracking-[0.22em] text-pink-500">
                {eyebrow}
            </p>
...
const inputClass =
    "w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-normal outline-none focus:border-pink-400 focus:ring-4 focus:ring-pink-50";