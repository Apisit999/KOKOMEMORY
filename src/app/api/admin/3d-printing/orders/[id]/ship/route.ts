...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) { try { const admin = await requireAdminApi(request); const { id } = await context.params; const body = await request.json() as { carrier?: string; trackingNumber?: string }; return NextResponse.json({ success: true, ...(await transitionThreeDOrder(id, "ship", admin.uid, body)) }); } catch (error) { const denied = authErrorResponse(error); if (denied) return denied; const code = error instanceof Error ? error.message : "ORDER_TRANSITION_FAILED"; return NextResponse.json({ success: false, error: code, code }, { status: code === "ORDER_NOT_FOUND" ? 404 : 409 }); } }
