...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
    try {
        ...
        if (typeof key !== "string" || !key.startsWith(`3d-payments/${orderId}/`) || key.includes("..")) return NextResponse.json({ error: "FILE_NOT_FOUND" }, { status: 404 });
    ...
    } catch (error) { const denied = authErrorResponse(error); if (denied) return denied; const code = error instanceof Error ? error.message : "PROOF_DOWNLOAD_FAILED"; return NextResponse.json({ error: code }, { status: code === "R2_PRIVATE_BUCKET_NOT_CONFIGURED" ? 503 : 500 }); }
}
