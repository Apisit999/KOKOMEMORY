...
export default function AccountLayout({ children }: { children: ReactNode }) {
    ...
    const navigation = (mobile = false) => <nav aria-label={locale === "th" ? "เมนูบัญชี" : "Account menu"} className={mobile ? "min-h-0 flex-1 overflow-y-auto space-y-6" : "space-y-6"}>
...
function breadcrumb(pathname: string, locale: "th" | "en") {
    ...
    return <nav aria-label={th ? "เส้นทางหน้า" : "Breadcrumb"} className="mx-auto flex max-w-6xl flex-wrap items-center gap-1.5 px-4 pt-5 text-xs text-slate-400 sm:px-8">{items.map((item, index) => <span key={`${item.label}-${index}`} className={isBooking && !item.href ? "inline-flex max-w-full min-w-0 items-start gap-1.5" : "inline-flex items-center gap-1.5"}>{index > 0 && <ChevronRight size={13} className={isBooking && !item.href ? "shrink-0" : undefined}/>} {item.href ? <Link href={item.href} className="font-semibold hover:text-[#D93687]">{item.label}</Link> : <span className={isBooking ? "min-w-0 break-all font-semibold text-slate-700" : "font-semibold text-slate-700"}>{item.label}</span>}</span>)}</nav>;
}
